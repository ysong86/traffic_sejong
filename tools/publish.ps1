# 세종시 교통안전 상황판 — 수집 → 생성 → 배포
#
# 왜 PC 에서 도는가
#   ITS 국가교통정보센터 API 는 해외 IP 에서 응답하지 않는다(연결 거부).
#   GitHub Actions 러너는 해외에 있어 소통·돌발상황을 못 받는다. 그래서 수집과
#   화면 생성은 국내 IP 인 이 PC 에서 하고, GitHub 은 만들어진 페이지를
#   서빙만 한다. 세종시 물환경 상황판과 같은 구조다.
#
# 어떻게 배포하는가
#   gh-pages 브랜치에 **커밋 하나만** 두고 매번 덮어쓴다(amend + force push).
#   매 실행마다 새 커밋을 쌓으면 파일 하나가 660KB 라 한 달이면 저장소가
#   기가 단위로 불어난다.
#
# 쓰는 법
#   powershell -ExecutionPolicy Bypass -File tools\publish.ps1
#   자동 실행 등록은 tools\install_startup.ps1
#
# 먼저 할 것
#   1) git init  후 origin 원격 설정 (예: git remote add origin https://github.com/<계정>/<저장소>.git)
#   2) GitHub 저장소 Settings → Pages → Source = gh-pages 브랜치
#   3) $siteUrl 을 그 주소로 바꾸기

#   -Demo 를 주면 인증키가 있어도 샘플로 만든다(화면 점검용).
...
$siteUrl = "https://ysong86.github.io/traffic_sejong/"

Set-Location $root

# 정지 스위치 — data 폴더에 PAUSED 파일이 있으면 아무것도 하지 않는다.
# 자동 실행을 지울 권한이 없을 때 권한 없이 즉시 멈추기 위한 장치다.
...
# ---------------------------------------------------------------- 1. 수집·생성
#
# 인증키가 아직 없으면 샘플로 만든다. 이렇게 해두면 키를 받기 전에도 화면을
# 띄워 둘 수 있고, config.json 에 키를 넣는 순간 다음 주기부터 저절로
# 실측으로 바뀐다 — 스크립트를 다시 손댈 필요가 없다.
# 샘플일 때는 화면 맨 위에 그렇다고 굵게 적힌다(build_dashboard 의 notice).
$hasKeys = $false
$cfgPath = Join-Path $root "config.json"
if (Test-Path $cfgPath) {
    try {
        $cfg = ConvertFrom-Json (Get-Content $cfgPath -Raw -Encoding utf8)
        foreach ($name in @("koroad", "its", "facility", "kma")) {
            $key = $cfg.$name.key
            if ($key -and -not $key.StartsWith("여기에")) { $hasKeys = $true }
        }
    } catch {
        Write-Host "     config.json 을 읽지 못했습니다: $_"
    }
}

if ($Demo -or -not $hasKeys) {
    if ($Demo) {
        Write-Host "[1/3] 샘플 데이터로 생성 (-Demo)"
    } else {
        Write-Host "[1/3] 인증키가 없어 샘플 데이터로 생성합니다"
        Write-Host "      config.json 에 키를 넣으면 다음 주기부터 실측으로 바뀝니다."
    }
    & $python run.py --demo --out site
} else {
    Write-Host "[1/3] 수집 및 상황판 생성"
    & $python run.py --collect --out site
}
if ($LASTEXITCODE -ne 0) { Fail "run.py 가 실패했습니다." }
...
if (-not (Test-Path $worktree)) {
    ...
    if ($LASTEXITCODE -ne 0) { Fail "워크트리를 만들지 못했습니다. git 저장소가 맞는지 확인하세요." }
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        if ($siteUrl) {
            Write-Host "     $siteUrl 갱신됨 ($stamp)"
        } else {
            Write-Host "     gh-pages 갱신됨 ($stamp). publish.ps1 의 `$siteUrl 을 채우면 주소가 표시됩니다."
        }
    }
} finally {
    Pop-Location
